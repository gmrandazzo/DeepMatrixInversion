...
def main():
    if len(sys.argv) != 5:
        print(f'Usage {sys.argv[0]} [dataset matrix to invert] [dataset matrix inverted] [dataset singular matrix to invert] [dataset singular matrix inverted]')
        exit(0)
    ...
    if verify_matrix_inversion(X, Y) and verify_pseudo_inversion(X_Singular, Y_Singular):
        print('Dataset valid.')
    else:
